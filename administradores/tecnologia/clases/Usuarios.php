<?php 
    ...
    class Usuarios extends Conexion {
        ...
        public function loginUsuario($usuario, $password) {
            ...
            $sql = "SELECT * FROM t_usuarios_sistemas
            ...
            if (mysqli_num_rows($respuesta) > 0) {
                ...
                if ($datosUsuarios['activo'] == 1) {
                    ...
                    $_SESSION['usuario']['id'] = $datosUsuarios['id_usuario_sis'];
        ...
        public function agregaNuevoUsuario($datos) {
            ...
            if ($idPersona > 0) {
                $sql = "INSERT INTO t_usuarios_sistemas (id_rol, id_persona, usuario, password, area)
                        VALUES (?, ?, ?, ?, ?)";
        ...
        public function obtenerDatosUsuario($idUsuario) {
            $conexion = Conexion::conectar();
            $sql = "SELECT usuarios.id_usuario_sis AS idUsuario, usuarios.usuario AS nombreUsuario,
                    ...
                    FROM t_usuarios_sistemas AS usuarios
                    ...
                    AND usuarios.id_usuario_sis = '$idUsuario'";
            $respuesta = mysqli_query($conexion, $sql);
        ...
        public function actualizarUsuario($datos) {
            ...
            if ($exitoPersona) {    
                $sql = "UPDATE t_usuarios_sistemas SET id_rol = ?, usuario = ?, area = ?
                        WHERE id_usuario_sis = ?";   
                $query = $conexion->prepare($sql);
        ...
        public function obtenerIdPersona($idUsuario) {
            $conexion = Conexion::conectar();
            $sql = "SELECT persona.id_persona AS idPersona FROM t_usuarios_sistemas AS usuarios
                    INNER JOIN t_persona AS persona ON usuarios.id_persona = persona.id_persona
                    AND usuarios.id_usuario_sis = '$idUsuario'";
            $respuesta = mysqli_query($conexion, $sql);
        ...
        public function resetPassword($datos) {
            $conexion = Conexion::conectar();
            $sql = "UPDATE t_usuarios_sistemas SET password = ? WHERE id_usuario_sis = ?";
            $query = $conexion->prepare($sql);
        ...
        public function cambioEstatusUsuario($idUsuario, $estatus) {
            ...
            $sql = "UPDATE t_usuarios_sistemas SET activo = ? WHERE id_usuario_sis = ?";
            $query = $conexion->prepare($sql);
        ...
        public function eliminarUsuario($datos) {
            ...
            if ($reportes == 0 && $asignaciones == 0) {
                $sql = "DELETE FROM t_usuarios_sistemas WHERE id_usuario_sis = ?";
                $query = $conexion->prepare($sql);
