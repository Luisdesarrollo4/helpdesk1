<?php 
    ...
    class Usuarios extends Conexion {
        ...
        public function loginUsuario($usuario, $password) {
            ...
            $sql = "SELECT * FROM t_usuarios_juridico
            ...
            if (mysqli_num_rows($respuesta) > 0) {
                ...
                if ($datosUsuarios['activo'] == 1) {
                    ...
                    $_SESSION['usuario']['id'] = $datosUsuarios['id_usuario_jur'];
        ...
        public function agregaNuevoUsuario($datos) {
            ...
            if ($idPersona > 0) {
                $sql = "INSERT INTO t_usuarios_juridico (id_rol, id_persona, usuario, password, area)
                        VALUES (?, ?, ?, ?, ?)";
        ...
        public function obtenerDatosUsuario($idUsuario) {
            $conexion = Conexion::conectar();
            $sql = "SELECT usuarios.id_usuario_jur AS idUsuario, usuarios.usuario AS nombreUsuario,
                    ...
                    FROM t_usuarios_juridico AS usuarios
                    ...
                    AND usuarios.id_usuario_jur = '$idUsuario'";
            $respuesta = mysqli_query($conexion, $sql);
        ...
        public function actualizarUsuario($datos) {
            ...
            if ($exitoPersona) {    
                $sql = "UPDATE t_usuarios_juridico SET id_rol = ?, usuario = ?, area = ?
                        WHERE id_usuario_jur = ?";   
                $query = $conexion->prepare($sql);
        ...
        public function obtenerIdPersona($idUsuario) {
            $conexion = Conexion::conectar();
            $sql = "SELECT persona.id_persona AS idPersona FROM t_usuarios_juridico AS usuarios
                    INNER JOIN t_persona AS persona ON usuarios.id_persona = persona.id_persona
                    AND usuarios.id_usuario_jur = '$idUsuario'";
            $respuesta = mysqli_query($conexion, $sql);
        ...
        public function resetPassword($datos) {
            $conexion = Conexion::conectar();
            $sql = "UPDATE t_usuarios_juridico SET password = ? WHERE id_usuario_jur = ?";
            $query = $conexion->prepare($sql);
        ...
        public function cambioEstatusUsuario($idUsuario, $estatus) {
            ...
            $sql = "UPDATE t_usuarios_juridico SET activo = ? WHERE id_usuario_jur = ?";
            $query = $conexion->prepare($sql);
        ...
        public function eliminarUsuario($datos) {
            ...
            if ($reportes == 0 && $asignaciones == 0) {
                $sql = "DELETE FROM t_usuarios_juridico WHERE id_usuario_jur = ?";
                $query = $conexion->prepare($sql);
